...
This is for what I use CLI most. As Developer eXperience Lead I do some demos and show some new customers how certain things are done in the [Platform of Trust](https://platformoftrust.net). In those cases I use CLI. So far I've managed without any specific "SDK-like" tool and used only ready-made scripts as examples. Some of the scripts use cURL, which is another command-line tool. Some are miniature Python apps which just make some API calls against Platform of Trust. The examples are collected in one "DX tests" folder. That's it. That's more MVP tool version. If you like cURL, you should also checkout [httpie](https://httpie.org/) 
