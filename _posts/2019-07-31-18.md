...
Piping multiple commands is a neat feature in CLI. You can put multiple tools in a chain of events in which one gets the input from previous one. Eventually you'll have one-liner using plethora of CLI tools and neat result. As a simple example formatting JSON response from API can be achieved with ```curl https://api-sandbox.oftrust.net/products/v1/ | python -m json.tool```. Or more simply with jq ``` curl https://api-sandbox.oftrust.net/products/v1/ | jq ```. 

jq is a lightweight and flexible command-line JSON processor. [jq is like sed for JSON data](https://stedolan.github.io/jq/) - you can use it to slice and filter and map and transform structured data with the same ease that sed, awk, grep and friends let you play with text.
