...
Technical growth strategies such as libraries, pluging and CLI tools aim at hiding the complexity from the AP product consuming developer. **Keep in mind the APIs should hide all of the complexities, internals and logic required to complete a task.** API should make the application developers life easier. If you want to be loved by developers, you’ll make working with your API a pleasant task. 
